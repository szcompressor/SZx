...
unsigned char* cuSZx_fast_compress_args_unpredictable_blocked_float(float *oriData, size_t *outSize, float absErrBound, size_t nbEle, int blockSize)
{
	...
	short* d_offsets;
    hipMalloc((void**)&d_meta, msz); 
    //checkCudaErrors(cudaMemcpy(d_meta, meta, msz, cudaMemcpyHostToDevice)); 
    hipMemset(d_meta, 0, msz);
    hipMalloc((void**)&d_offsets, nbBlocks*sizeof(short)); 
    hipMemset(d_offsets, 0, nbBlocks*sizeof(short));
    hipMalloc((void**)&d_midBytes, mbsz); 
    hipMemset(d_midBytes, 0, mbsz);

    high_resolution_clock::time_point t1 = high_resolution_clock::now();
    dim3 dimBlock(64, blockSize/64);
    dim3 dimGrid(65536, 1);
    const int sMemsize = blockSize * sizeof(float) + dimBlock.y * sizeof(int);
    hipLaunchKernelGGL(compress_float, dim3(dimGrid), dim3(dimBlock), sMemsize, 0, d_oriData, d_meta, d_offsets, d_midBytes, absErrBound, blockSize, nbBlocks, mSize);
    hipError_t err = hipGetLastError();        // Get error code
    printf("HIP Error: %s\n", hipGetErrorString(err));
    hipDeviceSynchronize();
    high_resolution_clock::time_point t2 = high_resolution_clock::now();
    duration<double> time_span = duration_cast<duration<double>>(t2 - t1);
    std::cout << "GPU compression time: " << time_span.count() << " seconds.\n";
    std::cout << std::endl;
    //printf("GPU compression timing: %f ms\n", timer_GPU.GetCounter());
    hipMemcpy(meta, d_meta, msz, hipMemcpyDeviceToHost); 
    hipMemcpy(offsets, d_offsets, nbBlocks*sizeof(short), hipMemcpyDeviceToHost); 
    hipMemcpy(midBytes, d_midBytes, mbsz, hipMemcpyDeviceToHost); 
    printf("test\n");
...
void cuSZx_fast_decompress_args_unpredictable_blocked_float(float** newData, size_t nbEle, unsigned char* cmpBytes)
{
	*newData = (float*)malloc(sizeof(float)*nbEle);
    ...
    hipMalloc((void**)&d_data, ncBlocks*blockSize*sizeof(float)); 
    hipMemcpy(d_data, data, ncBlocks*blockSize*sizeof(float), hipMemcpyHostToDevice); 

    high_resolution_clock::time_point t1 = high_resolution_clock::now();
    dim3 dimBlock(64, blockSize/64);
    dim3 dimGrid(65536, 1);
    const int sMemsize = blockSize * sizeof(float) + dimBlock.y * sizeof(int);
    hipLaunchKernelGGL(decompress_float, dim3(dimGrid), dim3(dimBlock), sMemsize, 0, d_data, blockSize, ncBlocks, mSize);
    hipError_t err = hipGetLastError();        // Get error code
    printf("HIP Error: %s\n", hipGetErrorString(err));
    hipDeviceSynchronize();
    high_resolution_clock::time_point t2 = high_resolution_clock::now();
    duration<double> time_span = duration_cast<duration<double>>(t2 - t1);
    std::cout << "GPU decompression time: " << time_span.count() << " seconds.\n";
    //printf("GPU decompression timing: %f ms\n", timer_GPU.GetCounter());
    hipMemcpy(data, d_data, ncBlocks*blockSize*sizeof(float), hipMemcpyDeviceToHost); 
