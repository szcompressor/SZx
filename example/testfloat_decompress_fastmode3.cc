...
//    nbEle = atoi(argv[2]);
    ...
    cost_start();
    float *data = NULL;
    //SZx_fast_decompress_args_unpredictable_blocked_randomaccess_float(&data, nbEle, bytes);
    //SZx_fast_decompress_args_unpredictable_blocked_randomaccess_float_openmp(&data, nbEle, bytes);
    data = (float*)SZx_fast_decompress_args_unpredictable_blocked_randomaccess_openmp(SZx_FLOAT, nbEle, bytes);
    cost_end();
