...
int main(int argc, char * argv[])
{
    ...
    float valueRange = computeValueRange_float(data, nbEle, &radius, &medianValue);
    float initErrorBound = valueRange*1E-3;
    cost_start();

    float errorBound = estimateErrorBoundbasedonCR_float(targetCompressionRatio, tolerance, data, initErrorBound, blockSize, nbEle);
    cost_end();
    printf("searched error bound = %.30G\n", errorBound);
    size_t outSize = 0;
    unsigned char* bytes = SZ_fast_compress_args_unpredictable_blocked_float(data, &outSize, errorBound, nbEle, blockSize);
