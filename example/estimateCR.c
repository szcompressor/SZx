...
int main(int argc, char * argv[])
{
    ...
    float* buffer = NULL;
    float approximateValueRange = computeRadiusBuffer_float(data, nbEle, samplingStride, blockSize, &radiusArray, &mediusArray, &buffer);    
    size_t sumReqNbB = 0, sum_actual_leadNum = 0;
    float CR = estimateCRbasedonErrorBound_buffered_float(errorBound, buffer, mediusArray, radiusArray, samplingStride, blockSize, nbEle, &sumReqNbB, &sum_actual_leadNum);	
    cost_end();
    printf("timecost=%f\n",totalCost); 
