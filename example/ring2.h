...
// int MPIR_Allgatherv_intra_ring_RI2(const void *sendbuf,
//                                   MPI_Aint sendcount,
...
// int MPI_Allreduce_SZx_FXR_RI2(const void *sendbuf,
//                              void *recvbuf,
//                              float compressionRatio,
//                              float tolerance,
//                              MPI_Aint count,
...
int MPIR_Allgatherv_intra_ring_RI2_record(const void *sendbuf,
                                         MPI_Aint sendcount,
                                         MPI_Datatype sendtype,
                                         void *recvbuf,
                                         const MPI_Aint *recvcounts,
...
int MPI_Allreduce_SZx_FXR_RI2_record(const void *sendbuf,
                                    void *recvbuf,
                                    float compressionRatio,
                                    float tolerance,
                                    MPI_Aint count,
                                    ...
                                    MPI_Comm comm)
{
    ...
    for (i = 0; i < nranks - 1; i++)
    {
        ...
        CPR_timer -= MPI_Wtime();
        SZ_fast_decompress_split(fast_mode, SZ_FLOAT, (unsigned char *)newData, tmpbuf, 0, 0, 0, 0, cnts[recv_rank]);
...
int MPIR_Allgatherv_intra_ring_RI2_oa_record(const void *sendbuf,
                                                MPI_Aint sendcount,
                                                MPI_Datatype sendtype,
                                                void *recvbuf,
                                                const MPI_Aint *recvcounts,
...
// mt here means multi-threads oa here means optimized allgather
int MPI_Allreduce_SZx_FXR_RI2_oa_record(const void *sendbuf,
                                           void *recvbuf,
                                           float compressionRatio,
                                           float tolerance,
                                           MPI_Aint count,
                                           ...
                                           MPI_Comm comm)
{
    ...
    /* Phase 3: Allgatherv ring, so everyone has the reduced data */
    mpi_errno = MPIR_Allgatherv_intra_ring_RI2_oa_record(MPI_IN_PLACE, -1, MPI_DATATYPE_NULL, recvbuf, cnts,
                                                            displs, datatype, comm, outputBytes, compressionRatio, tolerance, &ALLGATHER_timer, &CPR_timer);
    if (mpi_errno)
    {
        exit(-1);
