...
int MPIR_Allgatherv_intra_ring_record(const void *sendbuf,
                                      MPI_Aint sendcount,
                                      MPI_Datatype sendtype,
                                      void *recvbuf,
                                      const MPI_Aint *recvcounts,
                                      const MPI_Aint *displs,
                                      MPI_Datatype recvtype,
                                      MPI_Comm comm, double *ALLGATHER_timer)
{
    int comm_size, rank, i, left, right;
    int mpi_errno = MPI_SUCCESS;
    int mpi_errno_ret = MPI_SUCCESS;
...
int MPIR_Allreduce_intra_ring_record(const void *sendbuf,
                                     void *recvbuf,
                                     MPI_Aint count,
                                     MPI_Datatype datatype,
                                     MPI_Op op,
                                     MPI_Comm comm)
{
    ...
    /* Phase 3: Allgatherv ring, so everyone has the reduced data */
    mpi_errno = MPIR_Allgatherv_intra_ring_record(MPI_IN_PLACE, -1, MPI_DATATYPE_NULL, recvbuf, cnts,
                                                  displs, datatype, comm, &ALLGATHER_timer);
    if (mpi_errno)
    {
        exit(-1);
    }
    ...
    free(tmpbuf);
    MPI_total_timer += MPI_Wtime();
    double OTHER_timer = MPI_total_timer - ALLGATHER_timer - ReSca_timer - CPT_timer;
    double ReScanowait_timer = ReSca_timer - WAIT_timer;
    if (PRINT_DETAILS)
    {
        printf("For process %d, the Allgather time is %f, the Reduce_Scatter %f, the Reduce_Scatter(no wait) %f, the WAIT time is %f, the CPT time is %f, the OTHER time is %f, the total time is %f\n", rank, ALLGATHER_timer,
               ReSca_timer, ReScanowait_timer, WAIT_timer, CPT_timer, OTHER_timer, MPI_total_timer);
    }
    ...
    if (rank == 0)
    {
        ...
        {
            printf("For whole allreduce, the avg Allgather time is %f, the avg Reduce_Scatter time is %f, the avg Reduce_Scatter(no wait) time is %f, the avg WAIT time is %f, the avg CPT time is %f, the avg OTHER time is %f, the avg total time is %f\n", ALLGATHER_timer_all / nranks * 1000000,
                   ReSca_timer_all / nranks * 1000000, ReScanowait_timer_all / nranks * 1000000, WAIT_timer_all / nranks * 1000000, CPT_timer_all / nranks * 1000000, OTHER_timer_all / nranks * 1000000, MPI_total_timer_all / nranks * 1000000);
        }
        if (PRINT_EXPERIMENTS)
        {
            printf("%f, %f, %f, %f, %f, %f\n",
                   ALLGATHER_timer_all / nranks * 1000000, ReScanowait_timer_all / nranks * 1000000, WAIT_timer_all / nranks * 1000000, CPT_timer_all / nranks * 1000000, OTHER_timer_all / nranks * 1000000, MPI_total_timer_all / nranks * 1000000);
        }
    }
    return mpi_errno;
}

int MPIR_Allgatherv_intra_ring_record_save_midsteps(const void *sendbuf,
                                                    MPI_Aint sendcount,
                                                    MPI_Datatype sendtype,
                                                    void *recvbuf,
                                                    const MPI_Aint *recvcounts,
    ...
    { /* While we have data to send or receive */
        ...
        { /* If there's no data to send, just do a recv call */
            ...
            if (SAVE_CPR_RESULT == 1)
            {
                ...
                sprintf(outputFilePath, "%s/%d_%d.allagther", cmpPath, rank, step_counter);
                writeFloatData_inBytes(rbuf, recvnow, outputFilePath, &status);
                if (status != SZ_SCES)
                {
...
int MPIR_Allreduce_intra_ring_record_save_midsteps(const void *sendbuf,
                                                   void *recvbuf,
                                                   MPI_Aint count,
                                                   MPI_Datatype datatype,
                                                   MPI_Op op,
                                                   MPI_Comm comm)
{
    ...
    for (i = 0; i < nranks - 1; i++)
    {
        ...
        if (SAVE_CPR_RESULT == 1)
        {
            ...
            char outputFilePath[256];
            char cmpPath[256] = "/lcrc/project/sbi-fair/jiajun/MPI-Coll-SZx-data/16_allreduce_midsteps";
            sprintf(outputFilePath, "%s/%d_%d.reduce-scatter", cmpPath, rank, i);
            writeFloatData_inBytes((char *)recvbuf + displs[recv_rank] * extent, cnts[recv_rank], outputFilePath, &status);
            if (status != SZ_SCES)
            {
                printf("Error: %s cannot be written!\n", outputFilePath);
                exit(0);
    ...
    /* Phase 3: Allgatherv ring, so everyone has the reduced data */
    mpi_errno = MPIR_Allgatherv_intra_ring_record_save_midsteps(MPI_IN_PLACE, -1, MPI_DATATYPE_NULL, recvbuf, cnts,
                                                                displs, datatype, comm, &ALLGATHER_timer);
    if (mpi_errno)
    {
        exit(-1);
    ...
    { /* While we have data to send or receive */
        ...
        { /* If there's no data to send, just do a recv call */
            ...
            *MPI_timer += MPI_Wtime();
            ...
            MPI_Get_count(&status, MPI_BYTE, &byteLength);
            *CPR_timer -= MPI_Wtime();
            unsigned char *data = SZ_fast_decompress(fast_mode, SZ_FLOAT, rbuf, byteLength, 0, 0, 0, 0, recvnow);
            memcpy(rbuf, data, sizeof(recvtype) * recvnow);
            free(data);
                                    ...
                                    MPI_Comm comm)
{
    ...
    for (i = 0; i < nranks - 1; i++)
    {
        ...
        ReSca_timer -= MPI_Wtime();
        MPI_Get_count(&stas[0], MPI_BYTE, &byteLength);
        ReSca_timer += MPI_Wtime();
        CPR_timer -= MPI_Wtime();
        unsigned char *data = SZ_fast_decompress(fast_mode, SZ_FLOAT, tmpbuf, byteLength, 0, 0, 0, 0, cnts[recv_rank]);
        CPR_timer += MPI_Wtime();
    ...
    mpi_errno = MPIR_Allgatherv_intra_ring_RI_record(MPI_IN_PLACE, -1, MPI_DATATYPE_NULL, recvbuf, cnts,
                                                     displs, datatype, comm, compressionRatio, tolerance, &ALLGATHER_timer, &CPR_timer);
    if (mpi_errno)
    {
        exit(-1);
    }
...
int MPIR_Allgatherv_intra_ring_RI_oa_record(const void *sendbuf,
                                            MPI_Aint sendcount,
                                            MPI_Datatype sendtype,
                                            void *recvbuf,
                                            const MPI_Aint *recvcounts,
                                            ...
                                            double *CPR_timer)
{
    ...
    *CPR_timer -= MPI_Wtime();
    unsigned char *bytes;
    bytes = SZ_fast_compress_args(fast_mode, SZ_FLOAT, osbuf, &outSize,
                                  cpr_mode, absErrBound, relBoundRatio,
                                  compressionRatio, tolerance,
                                  0, 0, 0, 0, (recvcounts[sidx]));
    memcpy(sbuf, bytes, outSize);
    ...
    *CPR_timer -= MPI_Wtime();
    for (i = 0; i < comm_size; i++)
    {
        unsigned char *data = SZ_fast_decompress(fast_mode, SZ_FLOAT, (char *)temp_recvbuf + displs[i] * recvtype_extent, byteLength, 0, 0, 0, 0, recvcounts[i]);
        memcpy((char *)recvbuf + displs[i] * recvtype_extent, data, recvcounts[i] * recvtype_extent);
        free(data);
    }
    *CPR_timer += MPI_Wtime();
    free(temp_recvbuf);
...
int MPI_Allreduce_SZx_FXR_RI_oa_record(const void *sendbuf,
                                       void *recvbuf,
                                       float compressionRatio,
                                       float tolerance,
                                       MPI_Aint count,
                                       ...
                                       MPI_Comm comm)
{
    ...
    /* Phase 3: Allgatherv ring, so everyone has the reduced data */
    mpi_errno = MPIR_Allgatherv_intra_ring_RI_oa_record(MPI_IN_PLACE, -1, MPI_DATATYPE_NULL, recvbuf, cnts,
                                                        displs, datatype, comm, compressionRatio, tolerance, &ALLGATHER_timer, &CPR_timer);
    if (mpi_errno)
    {
        exit(-1);
