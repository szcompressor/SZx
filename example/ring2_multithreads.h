...
int MPIR_Allgatherv_intra_ring_RI2_mt_record(const void *sendbuf,
                                         MPI_Aint sendcount,
...
int MPI_Allreduce_SZx_FXR_RI2_mt_record(const void *sendbuf,
                                    void *recvbuf,
                                    float compressionRatio,
                                    float tolerance,
                                    MPI_Aint count,
                                    ...
                                    MPI_Comm comm)
{
    ...
    for (i = 0; i < nranks - 1; i++)
    {
        ...
        CPR_timer -= MPI_Wtime();
        SZ_fast_compress_args2(4, SZ_FLOAT, (char *)recvbuf + displs[send_rank] * extent,
        ...
        CPR_timer -= MPI_Wtime();
        SZ_fast_decompress_split(4, SZ_FLOAT, (unsigned char *)newData, tmpbuf, 0, 0, 0, 0, cnts[recv_rank]);
        // unsigned char *data = SZ_fast_decompress(4, SZ_FLOAT, tmpbuf, byteLength, 0, 0, 0, 0, cnts[recv_rank]);
        CPR_timer += MPI_Wtime();
        CPT_timer -= MPI_Wtime();
        mpi_errno =
    ...
    mpi_errno = MPIR_Allgatherv_intra_ring_RI2_mt_record(MPI_IN_PLACE, -1, MPI_DATATYPE_NULL, recvbuf, cnts,
                                                ...
                                                double *CPR_timer)
{
    ...
    *CPR_timer -= MPI_Wtime();
    SZ_fast_compress_args2(4, SZ_FLOAT, osbuf, &outSize,
                           sbuf,
                           cpr_mode, absErrBound, relBoundRatio,
                           compressionRatio, tolerance,
    ...
    *CPR_timer -= MPI_Wtime();
    for (i = 0; i < comm_size; i++)
    {
        SZ_fast_decompress_split(4, SZ_FLOAT, (char *)recvbuf + displs[i] * recvtype_extent, (char *)temp_recvbuf + displs[i] * recvtype_extent, 0, 0, 0, 0, recvcounts[i]);
    }
    *CPR_timer += MPI_Wtime();
    free(temp_recvbuf);
...
// mt here means multi-threads oa here means optimized allgather
int MPI_Allreduce_SZx_FXR_RI2_mt_oa_record(const void *sendbuf,
                                           void *recvbuf,
                                           float compressionRatio,
                                           float tolerance,
                                           MPI_Aint count,
