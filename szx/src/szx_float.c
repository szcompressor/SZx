...
//	writeFloatData(buffer, nbBlocks*blockSize, "1.csv", &status);
	...
	//unsigned char* bytes = SZ_fast_compress_args_unpredictable_blocked_float(oriData, outSize, errorBound, nbEle, blockSize);
	// SZ_fast_compress_args_unpredictable_blocked_float2(oriData, outSize, outputBytes, errorBound, nbEle, blockSize);
    SZ_fast_compress_args_unpredictable_blocked_randomaccess_float2_openmp(oriData, outSize, outputBytes, errorBound,
                                                               nbEle, blockSize);       
}
...
SZ_fast_compress_args_unpredictable_blocked_float_test(float *oriData, size_t *outSize, float absErrBound, size_t nbEle,
    ...
    *outSize += (3 + 1 + 1 + sizeof(size_t) + stateNBBytes + sizeof(float) * nbConstantBlocks);
    ...
    sz_cost_end_msg("sequential-1 compress");
    sz_cost_start();
    convertIntArray2ByteArray_fast_1b_args(stateArray, actualNBBlocks, r);
	sz_cost_end_msg("int to bytes");
    sz_cost_start();
    free(stateArray);
    free(medianArray);	
    free(radiusArray);
...
unsigned char *
SZ_fast_compress_args_unpredictable_blocked_randomaccess_float2_openmp_split(float *oriData, size_t *outSize, unsigned char* outputBytes, 
    unsigned char* chunk_arr, size_t chunk_iter, float absErrBound, size_t nbEle,
                                                  int blockSize) {
// #ifdef _OPENMP
    // printf("use openmp\n");
...
// #endif
    ...
    size_t *arr = (size_t*) chunk_arr; // The start of the chunk array for storing the compressed data size
