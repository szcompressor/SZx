 ...
 *  @date Aug, 2022
 *  @brief SZx_Init, Compression and Decompression functions
...
//	float preCR = CR;
	...
	if(foundFlag==0)
	{
		for(i=0;i<nbIterations;i++)
		{
			...
			CR = estimateCRbasedonErrorBound_buffered_double(errorBound, buffer, medianArray, radiusArray, samplingRate, blockSize, nbEle, &sumReqNbB, &sum_actual_leadNum);
...
float estimateErrorBoundbasedonCR_double(float targetCompressionRatio, float tolerance, double* data, float initErrorBound, int blockSize, size_t nbEle){
	...
	//search for a valid range for error bounds
	int i = 0;
	float CR = estimateCRbasedonErrorBound_double(errorBound, data, blockSize, nbEle);	
	int nbIterations = 15;
	if(fabs(targetCompressionRatio-CR) < tolerance)
		return errorBound;
	float left_error = 0, right_error = 0;	
	...
	if(targetCompressionRatio<CR)
	{
		...
		for(i=0;i<nbIterations;i++)
		{
			CR = estimateCRbasedonErrorBound_double(left_error, data, blockSize, nbEle);
	...
	else if(targetCompressionRatio>CR) //rightward search
	{
		...
		for(i=0;i<nbIterations;i++)
		{
			CR = estimateCRbasedonErrorBound_double(right_error, data, blockSize, nbEle);
			//printf("errorBound=%.30G, CR=%f\n", right_error, CR);			
	...
	if(foundFlag==0)
	{
		...
		{
			errorBound = (left_error+right_error)/2;
			CR = estimateCRbasedonErrorBound_double(errorBound, data, blockSize, nbEle);
			//printf("errorBound=%.30G, CR=%f\n", errorBound, CR);			
			if(fabs(CR-targetCompressionRatio) < targetTolerance || fabs(CR-preCR)<targetTolerance/2)
			{
				foundFlag = 1;
...
//	writeFloatData(buffer, nbBlocks*blockSize, "1.csv", &status);
	...
	float errorBound = estimateErrorBoundbasedonCR_buffered_double(compressionRatio, tolerance, samplingStride, initErrorBound, blockSize, nbEle, buffer, mediusArray, radiusArray);
	
	//unsigned char* bytes = SZx_fast_compress_args_unpredictable_blocked_float(oriData, outSize, errorBound, nbEle, blockSize);
	SZx_fast_compress_args_unpredictable_blocked_double2(oriData, outSize, outputBytes, errorBound, nbEle, blockSize);
    return outputBytes;                                                                    
}
...
SZx_fast_compress_args_unpredictable_blocked_double(double *oriData, size_t *outSize, float absErrBound, size_t nbEle,
    ...
    *outSize += (3 + 1 + 1 + sizeof(size_t) + stateNBBytes + sizeof(float) * nbConstantBlocks);
    ...
    for (i = 0; i < nbBlocks; i++, op += blockSize) {
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_double(op, blockSize, absErrBound, q, &oSize,
    ...
    if (remainCount != 0) {
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_double(op, remainCount, absErrBound, q, &oSize,
                                                                leadNumberArray_int, medianArray[i], radiusArray[i]);
            *outSize += oSize;
        } else {
            floatToBytes(p, medianArray[i]);
...
unsigned char *
SZx_fast_compress_args_unpredictable_blocked_randomaccess_double_openmp(double *oriData, size_t *outSize, float absErrBound,
...
#pragma omp for reduction(+:nbNonConstantBlocks) schedule(static)
    for (i = 0; i < nbBlocks; i++) {
        ...
        computeStateMedianRadius_double2(op + i * blockSize, blockSize, absErrBound, stateArray + i, medianArray + i,
                                        &radius);
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_double(op + i * blockSize, blockSize, absErrBound,
...
//    exit(0);
    if (remainCount != 0) {
        ...
        computeStateMedianRadius_double2(op + i * blockSize, remainCount, absErrBound, stateArray + i, medianArray + i,
                                        &radius);
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_double(op + i * blockSize, remainCount, absErrBound,
                                                                tmp_q + i * blockSize * sizeof(float), outSizes + i,
...
unsigned char *
SZx_fast_compress_args_unpredictable_blocked_randomaccess_double(double *oriData, size_t *outSize, float absErrBound,
    size_t nbEle, int blockSize) {
    double *op = oriData;
...
SZx_fast_compress_args_unpredictable_double(double *data, size_t *outSize, float absErrBound, size_t r5, size_t r4,
    ...
    size_t dataLength = SZx_computeDataLength(r5, r4, r3, r2, r1);
