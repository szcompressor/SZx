...
unsigned char *
SZx_fast_compress_args_with_prediction_float(float *pred, float *data, size_t *outSize, float absErrBound, size_t r5,
    ...
    unsigned char *output = SZx_fast_compress_args_unpredictable_float(delta, outSize, absErrBound, r5, r4, r3, r2, r1,
                                                                      medianValue, radius);
    return output;
}
...
//	writeFloatData(buffer, nbBlocks*blockSize, "1.csv", &status);
	...
	SZx_fast_compress_args_unpredictable_blocked_float2(oriData, outSize, outputBytes, errorBound, nbEle, blockSize);
                                                  ...
                                                  int blockSize)
{
    ...
    for (i = 0; i < nbBlocks; i++, op += blockSize) {
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_float(op, blockSize, absErrBound, q, &oSize,
    ...
    if (remainCount != 0) {
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_float(op, remainCount, absErrBound, q, &oSize,
                                                                leadNumberArray_int, medianArray[i], radiusArray[i]);
            outSize += oSize;
        } else {
            floatToBytes(p, medianArray[i]);
...
#pragma omp for reduction(+:nbNonConstantBlocks) schedule(static)
    for (i = 0; i < nbBlocks; i++) {
        ...
        computeStateMedianRadius_float2(op + i * blockSize, blockSize, absErrBound, stateArray + i, medianArray + i,
                                        &radius);
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_float(op + i * blockSize, blockSize, absErrBound,
...
//    exit(0);
    if (remainCount != 0) {
        ...
        computeStateMedianRadius_float2(op + i * blockSize, remainCount, absErrBound, stateArray + i, medianArray + i,
                                        &radius);
        if (stateArray[i]) {
            SZx_fast_compress_args_unpredictable_one_block_float(op + i * blockSize, remainCount, absErrBound,
                                                                tmp_q + i * blockSize * sizeof(float), outSizes + i,
